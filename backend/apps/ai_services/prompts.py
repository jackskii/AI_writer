...
AUTO_EDIT_PREFILLS = {
    '增加细节': '''请在不改变剧情事实、人物关系、叙事视角和时间线的前提下，优化这段文本。

默认处理原则：
- 润色对话和描写，使其更符合角色性格与当下情境。
- 适当增加对话与描写，让场景更流畅、更有画面感。
- 不要过度扩写，不要为了细节而堆砌细节。
- 不需要详细描写所有场景，好的小说应该张弛有度。
...
- （展开，<具体要求>）：按要求展开该段内容。
  ...
  - （展开，500字左右）  
  - （展开，不超过5句对话）  
- 未标记“展开”的段落，按默认处理原则执行，不做大幅扩写。

硬性要求：
1. 严格理解并尽量满足“展开”标签中的具体要求（字数、句数、重点、风格等）。
2. 若标签要求与剧情连贯性冲突，优先保证剧情连贯，再在约束范围内尽量满足标签。
3. 不新增原文没有的关键剧情结论，不改写人物立场。
4. 输出时删除所有控制标签，不得在结果中出现“（展开，...）”等标记。
5. 最终只输出正文，不要附加解释、说明或分析。''',
